/* Задача 4 . Алгоритъмът на Флойд
Уоршал се използва за намиране на най къс път, както в ориентиран така и в
...
графа.
Приемаме,че имаме претеглен граф G=(V,E) V
брой върхове, Е брой ребра от тип.
...
A
k [i,j]=min(A k 1 [i,j], A k 1 [i,k]+A k 1 [k,
Това е пример от къде се взимат стойностите
A
1 [i,j]=min(A 0 [i,j], A 0 [i,1]+A 0 [1,
Oт тази формула произлиза и основното неравенство:
if (A[i][k] + A[k][j] < A[i][j])
A[i][j] = A[i][k] + A[k][j];
Алгоритъмът е много приятен и лесен за запомняне, три последователни (вложени) цикъла, сложност О(N
3 ) и
...
A[i][j] = A[i][k] + A[k][j];
Индексът на
A k показва колко пъти ще се пренарежда мартрицата до намиране на минималните разстояния.
Колко са върховете, толкова цикълът ще трябва да направи променливата к , която трябва да бъде най
външният цикъл, защото “върти” цялата матрица, а i редове, j колоните. */
#include <stdio.h>
#define N 6
#define INF 9999
int main()
